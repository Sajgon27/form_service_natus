...
(function ($) {
        ...
        .then(function (data) {
          if (data && data.success) {
            ...
            console.error(
              ...
              data ? data.data.message : "Unknown error"
            );
            hideFormLoadingMessage();
          }
        })
  ...
  function showFormLoadingMessage() {
    const loadingHtml =
      '<div id="serwis-natu-loading" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: rgba(255,255,255,0.8); z-index: 999; display: flex; align-items: center; justify-content: center;">' +
  ...
  function populateForm(data) {
    ...
    $(`input[name="tryb_wspolpracy"][value="${modeValue}"]`)
      .prop("checked", true)
      .trigger("change");

    // Calculate the number of aquariums based on the actual data
    const aquariumCount = data.aquariums
      ? Object.keys(data.aquariums).length
      : 1;
  ...
  function ensureAquariumSectionsExist(count) {
    return new Promise((resolve) => {
      ...
      const checkInterval = setInterval(() => {
        ...
        console.log(
          `Waiting... Now have ${currentSections}/${count} aquarium sections`
        );
      ...
      setTimeout(() => {
        ...
        console.log(
          `Timeout reached. Have ${finalSections}/${count} aquarium sections. Continuing anyway.`
        );
        clearInterval(checkInterval);
        resolve();
  ...
  function populateAquariumSection(index, aquariumData) {
    ...
    if (!$section.length) {
      console.warn(
        `Aquarium section ${index} not found with selector "${sectionId}"`
      );
      ...
      for (const selector of alternativeSelectors) {
        ...
        if (altSection.length) {
          console.log(
            `Found aquarium section using alternative selector: ${selector}`
          );
          $section = altSection;
          break;
        }
      ...
      if (!$section.length) {
        console.error(
          `Could not find aquarium section ${index} with any selector. Aborting population.`
        );
        return;
      }
    }
      ...
      $section.find('input[name^="akw"][name$="[typ][]"]').each(function () {
        ...
        tankTypes.forEach((type) => {
          if (
            ...
            console.log(
              `Set tank type "${type}" using checkbox with value "${value}"`
            );
          }
        });
      });
      ...
      $section.find('input[name^="akw"][name$="[cel][]"]').each(function () {
        ...
        purposes.forEach((purpose) => {
          if (
            ...
            $checkbox.prop("checked", true).trigger("change");
            console.log(
              `Set purpose "${purpose}" using checkbox with value "${value}"`
            );
          }
        });
      });
      ...
      $section.find('input[name^="akw"][name$="[zakres][]"]').each(function () {
        ...
        scopes.forEach((scope) => {
          if (
            ...
            $checkbox.prop("checked", true).trigger("change");
            console.log(
              `Set scope "${scope}" using checkbox with value "${value}"`
            );
          }
        });
      });
    ...
    if (aquariumData["Dodatkowe usługi"]) {
      ...
      console.log(
        `Setting additional services for aquarium ${index}:`,
        additionalServices
      );
      ...
      additionalServices.forEach((service) => {
        ...
        if (service.toLowerCase().includes("badanie")) {
          ...
          if ($badanieCheckbox.length) {
            ...
            console.log(
              `Set additional service "${service}" using "badanie" checkbox`
            );
            matchFound = true;
          }
        }
        ...
        if (!matchFound) {
          $section.find('input[type="checkbox"]').each(function () {
            ...
            if (
              ...
              $checkbox.prop("checked", true).trigger("change");
              console.log(
                `Set additional service "${service}" using checkbox with value "${value}"`
              );
              matchFound = true;
        ...
        if (!matchFound) {
          console.warn(
            `Could not find matching checkbox for additional service "${service}" in aquarium ${index}`
          );
        }
      });
    }
    ...
    if (aquariumData.inne) {
      console.log(
        `Setting other information for aquarium ${index}:`,
        aquariumData.inne
      );
      ...
      $section.find('input[name^="akw"][name$="[inne][]"]').each(function () {
        ...
        inneValues.forEach((inneValue) => {
          if (
            ...
            (dataText &&
              dataText.toLowerCase().includes(inneValue.toLowerCase()))
          ) {
            $checkbox.prop("checked", true).trigger("change");
            console.log(
              `Set other information "${inneValue}" using checkbox with value "${value}"`
            );
          }
        });
      });
