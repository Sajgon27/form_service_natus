...
function register_orders_admin_page() {
    add_menu_page(
        'Zamówienia akwariów',          // Page title
        'Zamówienia',                   // Menu title
        'manage_options',               // Capability
        'aquarium_orders',              // Menu slug
        'display_orders_admin_page',    // Callback function
...
function display_orders_admin_page() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'serwis_natu_orders'; // zmień na swoją tabelę
    ...
    if ($results) {
        foreach ($results as $row) {
            ...
            echo '<td>'.esc_html($row['additional_notes']).'</td>';
